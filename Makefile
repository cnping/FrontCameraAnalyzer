...
components_linux: linux cmem osal syslink_hlos syslink_driver sgx-driver omx camera_analyzer gstomx media-controller-utils rpe_linux 
components_dsp: syslink_rtos edma3lld rpe_dsp

#==============================================================================
# Install components
#==============================================================================
components_install: linux_install syslink_install cmem_install sgx-driver_install omx_install camera_analyzer_install gstomx_install media-controller-utils_install rpe_install

#==============================================================================
# Clean up the targets built by 'make all'.
#==============================================================================
components_clean: linux_clean syslink_clean cmem_clean sgx-driver_clean omx_clean camera_analyzer_clean edma3lld_clean gstomx_clean media-controller-utils_clean rpe_clean
...
help:
	...
	@echo
	@echo "    camera_analayzer               : Build Camera Analyzer and OMX IL Clients"
	@echo "    camera_analyzer_clean          : Remove OMX generated files"
	@echo "    camera_analyzer_install        : Install OMX IL Clients"
	@echo
...
################################################################################
# Build Camera Analyzer and OMX ILClient Applications 
################################################################################

camera_analyzer:
	$(MAKE) -C $(APPS_INSTALL_DIR)/src $(APPS_BUILD_VARS) libs
	$(MAKE) -C $(APPS_INSTALL_DIR)/src $(APPS_BUILD_VARS) DEST_ROOT=$(APPS_INSTALL_DIR)/bin examples
	@echo "  CCLD   lib/libtiomx.so"
	@$(CSTOOL_PREFIX)gcc -Wl,-soname,libOMX_Core -shared -Xlinker --start-group \
	$(APPS_INSTALL_DIR)/lib/omxcore.av5T \
	$(APPS_INSTALL_DIR)/lib/domx.av5T \
	$(APPS_INSTALL_DIR)/lib/memcfg.av5T \
	$(APPS_INSTALL_DIR)/lib/timmosal.av5T \
	$(APPS_INSTALL_DIR)/lib/domx_delegates_shmem.av5T \
	$(APPS_INSTALL_DIR)/lib/omxcfg.av5T \
	$(SYSLINK_INSTALL_DIR)/packages/ti/syslink/lib/syslink.a_debug \
	$(FC_INSTALL_DIR)/packages/ti/sdo/rcm/lib/debug/rcm_syslink.av5T \
	$(FC_INSTALL_DIR)/packages/ti/sdo/fc/memutils/lib/release/memutils.av5T \
	$(FC_INSTALL_DIR)/packages/ti/sdo/fc/global/lib/debug/fcsettings.av5T \
	...
	$(UIA_INSTALL_DIR)/packages/ti/uia/linux/lib/servicemgr.a -Xlinker --end-group \
		...
		-Wl,-u,OMX_FreeHandle \
		-Wl,-u,OMX_GetRolesOfComponent \
		-pthread -lrt -o $(APPS_INSTALL_DIR)/lib/libtiomx.so.5.2 -Wl,--fix-cortex-a8 $(CFLAGS) $(LDFLAGS)

camera_analyzer_clean:
	rm -rf $(APPS_INSTALL_DIR)/bin ${APPS_INSTALL_DIR}/lib
	$(MAKE) -C $(APPS_INSTALL_DIR)/src $(APPS_BUILD_VARS) clean

camera_analyzer_install:
	install -d $(EXEC_DIR)/usr/share/ti/ti-omx
	for i in `find ${APPS_INSTALL_DIR}/bin -name "*.xv5T"`; do \
		install $$i ${EXEC_DIR}/usr/share/ti/ti-omx; \
	done
	install -d $(EXEC_DIR)/usr/lib
	install $(APPS_INSTALL_DIR)/lib/libtiomx.so.5.2 $(EXEC_DIR)/usr/lib/libtiomx.so.5.2
	ln -sf libtiomx.so.5.2 $(EXEC_DIR)/usr/lib/libtiomx.so.5
	ln -sf libtiomx.so.5.2 $(EXEC_DIR)/usr/lib/libtiomx.so
	ln -sf libtiomx.so.5.2 $(EXEC_DIR)/usr/lib/libOMX_Core.so
	ln -sf libtiomx.so.5.2 $(EXEC_DIR)/usr/lib/libOMX_Core
